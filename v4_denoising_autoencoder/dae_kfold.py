...
def run():
    ...
    for train_indices, test_indices in kfold.split(X_TRAIN):
        x_train_kfold = X_TRAIN[train_indices]
        x_test_kfold = X_TRAIN[test_indices]
        sess.run(optimizer, feed_dict={autoencoder['x']: x_train_kfold, autoencoder['corrupt_prob']: [1.0]})
        cost = sess.run(autoencoder['cost'], feed_dict={autoencoder['x']: x_test_kfold, autoencoder['corrupt_prob']: [1.0]})
        print(cost)
    # %%
